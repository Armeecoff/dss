...
async def get_all_users() -> List[int]:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT DISTINCT user_id FROM user_subscriptions") as cur:
            rows = await cur.fetchall()
            return [r[0] for r in rows]

async def get_stats() -> Dict:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT COUNT(*) FROM user_subscriptions") as cur:
        ...
        async with db.execute("SELECT COUNT(*) FROM saved_media") as cur:
